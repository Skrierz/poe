...
def main():
    account_name = input('Введите имя Вашего аккаунта: ')
    realm = input('Введите ваш реалм ("pc", "ps4"(?), "xbox"(?)): ')
    character_name = input('Введите имя Вашего персонажа: ')

    gems = get_equipped_gems_requirements(get_character_data(account_name, realm, character_name))
